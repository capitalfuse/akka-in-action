...
class LocalWordsSpec extends TestKit(ActorSystem("test"))
                        ...
                        with ImplicitSender {
  val receptionist: ActorRef = system.actorOf(Props[TestReceptionist](), JobReceptionist.name)
  ...
  "The words system" must {
    ...
    "count many occurences of words in a text" in {
      ...
      receptionist ! JobRequest("test3", (1 to 100).flatMap(_ => words ++ words).toList)
